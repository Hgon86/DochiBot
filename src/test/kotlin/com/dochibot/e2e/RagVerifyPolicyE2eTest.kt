...
/**
 * Verify가 실패하는 상황에서 정책 라우팅이 동작하고, LLM 호출이 차단되는지 E2E로 검증한다.
 */
@Testcontainers(disabledWithoutDocker = true)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@AutoConfigureWebTestClient
@ActiveProfiles("test")
class RagVerifyNoEvidenceE2eTest {
    ...
    companion object {
        ...
        fun registerProperties(registry: DynamicPropertyRegistry) {
            ...
            // Verify 활성화 + 강한 cutoff로 항상 정책 라우팅되도록 한다.
            registry.add("dochibot.rag.verify.enabled") { "true" }
            registry.add("dochibot.rag.verify.policy") { "NO_EVIDENCE" }
            registry.add("dochibot.rag.verify.min-top1-final-score") { "100.0" }
        }
    }
    ...
    @Test
    fun `verify 실패 시 NO_EVIDENCE 정책으로 라우팅하고 LLM 호출을 차단한다`() = runBlocking {
        ...
        assertEquals("[]", last.citationsJson)
    }
...
/**
 * Verify가 실패하는 상황에서 ASK_FOLLOWUP 정책 응답이 반환되고, LLM 호출이 차단되는지 E2E로 검증한다.
 */
@Testcontainers(disabledWithoutDocker = true)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@AutoConfigureWebTestClient
@ActiveProfiles("test")
class RagVerifyAskFollowUpE2eTest {
    ...
    companion object {
        ...
        fun registerProperties(registry: DynamicPropertyRegistry) {
            ...
            registry.add("dochibot.rag.verify.enabled") { "true" }
            registry.add("dochibot.rag.verify.policy") { "ASK_FOLLOWUP" }
            registry.add("dochibot.rag.verify.min-top1-final-score") { "100.0" }
        }
    }
    ...
    @Test
    fun `verify 실패 시 ASK_FOLLOWUP 정책으로 라우팅하고 LLM 호출을 차단한다`() = runBlocking {
        val userId = UUID.randomUUID()
        val token = issueToken(userId = userId, role = "ADMIN")
